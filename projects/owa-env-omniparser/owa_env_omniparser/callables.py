    ...
    try:
        ...
        if omniparser_path:
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    "omniparser",
                    str(omniparser_path / "util" / "omniparser.py")
                )
        ...
        try:
            ...
            if omniparser_dir.exists():
                spec = importlib.util.spec_from_file_location(
                    "omniparser",
                    str(omniparser_dir / "util" / "omniparser.py")
                )
                if spec and spec.loader:
                    omniparser_module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(omniparser_module)
        ...
        raise ImportError(
            "Failed to import OmniParser. Please install OmniParser or set OMNIPARSER_SOURCE_PATH."
        )
