...
class OmniParserAPIClient:
    ...
    def _load_config_from_env(self) -> OmniParserAPIConfig:
        ...
        return OmniParserAPIConfig(api_url=api_url, timeout=timeout, max_retries=max_retries, retry_delay=retry_delay)
    ...
    def parse_screen(self, screen_image: Union[str, bytes, BytesIO]) -> Dict[str, Any]:
        ...
        for attempt in range(self.config.max_retries):
            try:
                logger.info(
                    f"Calling OmniParser API at {self.config.api_url} (attempt {attempt + 1}/{self.config.max_retries})"
                )

                response = self.session.post(self.config.api_url, json=payload, timeout=self.config.timeout)
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if (
                            ...
                            or "som_image_base64" not in result
                            or "parsed_content_list" not in result
                        ):
