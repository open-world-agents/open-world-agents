...
def test_overlapping_key_presses():
    ...
    manager.handle_key_event("press", VK.KEY_A, base_time + 4000000000)  # A pressed again at 4s (should not create new subtitle)
    manager.handle_key_event("press", VK.KEY_B, base_time + 6000000000)  # B pressed again at 6s (should not create new subtitle)
    ...
    assert abs(a_duration_seconds - expected_a_duration) < 0.001, f"Expected A duration {expected_a_duration}s, got {a_duration_seconds}s"
    ...
    assert abs(b_duration_seconds - expected_b_duration) < 0.001, f"Expected B duration {expected_b_duration}s, got {b_duration_seconds}s"
