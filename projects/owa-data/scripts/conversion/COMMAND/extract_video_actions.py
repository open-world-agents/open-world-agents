...
def process_image(
    msg: sensor_msgs__msg__CompressedImage | sensor_msgs__msg__Image, target_size: Tuple[int, int] = (672, 378)
) -> Image.Image:
    """
...
def extract_rgb_front_images(
    bagfile: Path, topics_config: Dict, target_fps: float = 10.0
    ...
    with AnyReader([bagfile], default_typestore=typestore) as reader:
        ...
        image_topics = [
            ...
            if ("rgb" in t.lower() or "image" in t.lower() or "camera" in t.lower())
            and "camera_info" not in t.lower()
            and ("compressed" in t.lower() or "image_raw" in t.lower())
        ]
        ...
        for connection, timestamp, rawdata in tqdm(
            reader.messages(connections=connections), desc=f"Extracting images from {bagfile.name}", disable=False
        ):
