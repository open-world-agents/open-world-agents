...
def test_encoder_functionality(encoder, encoder_name: str, test_events: List[Dict[str, Any]]):
    ...
    try:
        ...
        success_count = sum(1 for orig, decoded in zip(test_events, decoded_events) 
...
def main():
    ...
    print("\nRecommendations:")
    print("- EventEncoder: Best for general MLLM training")
    ...
    if benchmark_results:
        fastest = min((name for name, result in benchmark_results.items() if result), 
                     key=lambda name: benchmark_results[name]['total_time'])
        print(f"- Fastest encoder: {fastest}")
        
        smallest_vocab = min((name for name, result in benchmark_results.items() 
                            if result and result['vocab_size']), 
