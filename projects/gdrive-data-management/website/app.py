...
def get_database_stats():
    ...
    try:
        ...
        # Duration statistics
        cursor.execute(
            "SELECT SUM(duration_seconds) as total_duration, SUM(accepted_duration) as total_accepted FROM dataset_analysis"
        )
        ...
        cursor.execute(
            "SELECT original_version, COUNT(*) as count FROM dataset_analysis GROUP BY original_version ORDER BY count DESC"
        )
        version_dist = cursor.fetchall()

        # Migration status
        cursor.execute(
            "SELECT migrated_version, COUNT(*) as count FROM dataset_analysis WHERE migrated_version IS NOT NULL GROUP BY migrated_version ORDER BY count DESC"
        )
        migration_dist = cursor.fetchall()

        # Game distribution (only available files)
        cursor.execute(
            "SELECT game_name, COUNT(*) as count FROM dataset_analysis WHERE available = 1 GROUP BY game_name ORDER BY count DESC LIMIT 10"
        )
...
def index():
    ...
    if not stats:
        return render_template(
            "error.html", error="Could not connect to database. Make sure dataset_analysis.db exists."
        )
...
def api_files():
    ...
    try:
        ...
        for file in files:
            ...
            file["efficiency"] = (
                round((file["accepted_duration"] / file["duration_seconds"] * 100), 1)
                if file["duration_seconds"] and file["duration_seconds"] > 0
                else 0
            )
...
def analytics_page():
    ...
    stats = get_database_stats()
    if not stats:
        return render_template("error.html", error="Could not connect to database for analytics.")
