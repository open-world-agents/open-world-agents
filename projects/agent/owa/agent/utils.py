...
def patch_unsloth_gradient_checkpointing():
    ...
    if torch.utils.checkpoint.checkpoint.__name__ == "unsloth_offloaded_gradient_checkpoint":
        return
...
def patch_gradient_checkpointing():
    ...
    if torch.utils.checkpoint.checkpoint.__name__ == "unsloth_gradient_checkpoint":
        return
    torch.utils.checkpoint._old_checkpoint = torch.utils.checkpoint.checkpoint
...
def initialize_unsloth_gradient_checkpointing(dtype=None):
    ...
    BACKWARD_PASS = True
    EXTRA_STREAMS = tuple(
        [torch.cuda.Stream() if DEVICE_TYPE == "cuda" else torch.xpu.Stream() for i in range(n_gpus)]
    )
    if DEVICE_TYPE == "cuda":
...
def unpatch_unsloth_smart_gradient_checkpointing():
    # All Unsloth Zoo code licensed under LGPLv3
    if (torch.utils.checkpoint.CheckpointFunction.__name__ == "UnslothCheckpointFunction") and hasattr(
