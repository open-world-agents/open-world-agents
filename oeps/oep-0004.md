...
❌ FAIL: Some plugins below quality thresholds (PASS: 80%/60%, FAIL: 60%/0%)
$ echo $?
1
...
⚠️  example plugin: 7/7 documented (100%), 3/7 good quality (43%)
❌ FAIL: Minimum plugin quality 43% (example) below threshold (60%)
$ echo $?
1
...
❌ FAIL: Minimum plugin quality 43% (example) below threshold (100%)
$ echo $?
1

# Custom coverage thresholds
$ owl env validate-docs --min-coverage-pass 0.9 --min-coverage-fail 0.7
❌ FAIL: Minimum plugin coverage 80% (desktop) below threshold (90%)
$ echo $?
1
...
$ owl env validate-docs --format json
{
  ...
  "thresholds": {
    "min_coverage_pass": 0.8,
    "min_quality_pass": 0.6
  },
  "plugins": {
    "std": {
      "documented": 2, "total": 2, "coverage": 1.0,
      "good_quality": 2, "quality_ratio": 1.0, "skipped": 0,
      "status": "pass", "issues": []
    },
    "example": {
      "documented": 7, "total": 7, "coverage": 1.0,
      "good_quality": 3, "quality_ratio": 0.43, "skipped": 0,
      "status": "warning", "issues": []
    },
    "desktop": {
      "documented": 20, "total": 25, "coverage": 0.8,
      "good_quality": 12, "quality_ratio": 0.48, "skipped": 0,
      "status": "warning", "issues": []
    },
    "custom": {
      "documented": 1, "total": 5, "coverage": 0.2,
      "good_quality": 0, "quality_ratio": 0.0, "skipped": 0,
      "status": "fail", "issues": ["custom/component1: Missing docstring"]
    }
  },
  "exit_code": 1
}
...
**Note**: The mkdocstrings handler is implemented as a separate package `mkdocstrings-owa` available on PyPI, not in this codebase.
...
**Note**: The OWA mkdocstrings handler is distributed as the `mkdocstrings-owa` package on PyPI and is automatically available when installed.
...
**Note**: The OWA handler is distributed as the `mkdocstrings-owa` package and is automatically available when installed via pip. It integrates seamlessly with the existing mkdocstrings workflow.
...
def test_plugin_documentation():
    ...
    assert data["min_plugin_coverage"] >= 0.9, f"Minimum plugin coverage {data['min_plugin_coverage']} below 90%"
...
$ owl env validate-docs --strict
✅ std plugin: 2/2 documented (100%), 2/2 good quality (100%)
✅ example plugin: 7/7 documented (100%), 7/7 good quality (100%)
✅ desktop plugin: 25/25 documented (100%), 25/25 good quality (100%)
⚠️  gst plugin: 3/4 documented (75%), 3/4 good quality (75%)

📊 Overall: Minimum plugin coverage: 75% (gst), Minimum plugin quality: 75% (gst)
❌ FAIL: Minimum plugin coverage 75% (gst) below threshold (100%)
...
**Note**: The mkdocstrings handler is implemented as the separate `mkdocstrings-owa` package available on PyPI.

**Core Components:**
- `projects/owa-core/owa/core/documentation/validator.py` - Documentation validation engine
- `mkdocstrings-owa` (external package) - Custom mkdocstrings handler for OWA plugins
...
- Lightweight custom handler for OWA plugin structure (via `mkdocstrings-owa` package)
- Automatic discovery through PyPI package installation
